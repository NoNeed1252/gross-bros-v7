export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      await streamOpenRouter({ messages: openAiMessages, model: 'google/gemini-2.0-flash-exp:free' });
      return;
    } catch (error) {
    ...
    try {
      await streamOpenRouter({ messages: openAiMessages, model: 'qwen/qwen-2.5-72b-instruct:free' });
      return;
    } catch (error) {
