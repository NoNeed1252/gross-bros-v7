export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      await streamOpenRouter({ messages: openAiMessages, model: 'deepseek/deepseek-chat:free' });
      return;
    } catch (error) {
    ...
    try {
      await streamOpenRouter({ messages: openAiMessages, model: 'qwen/qwen-2-7b-instruct:free' });
      return;
    } catch (error) {
